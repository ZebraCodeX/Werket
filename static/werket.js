(function () {
  ...
  function findInDocument(direction) {
    ...
    if (index >= 0) { editor.focus(); editor.setSelectionRange(index, index + query.length); $('findCount').textContent = 'Found'; }
  ...
  function renderKeyboard() {
    var host = $('keyboard'), layout = [7, 8, 8, 8], cursor = 0, html = '<div class="keyboard-suggest-row" id="keyboardSuggestions"><span class="empty">Type to see dictionary suggestions</span></div><div class="order-tray" id="orderTray" hidden></div><div class="keyboard-hint">Tap a family for its seven orders · the small label is its phonetic key</div>';
    ...
    host.innerHTML = html;
    host.querySelectorAll('[data-family]').forEach(function (button) { button.onclick = function (event) { event.stopPropagation(); var family = button.dataset.family, tray = $('orderTray'); tray.hidden = false; tray.innerHTML = '<span class="order-label">' + family + ' family</span>'; orders.forEach(function (order, index) { var choice = document.createElement('button'); choice.textContent = String.fromCodePoint(family.codePointAt(0) + index); choice.title = order === 'silent' ? 'Sixth order' : order; choice.onclick = function () { insert(choice.textContent); tray.hidden = true; }; tray.appendChild(choice); }); }; });
  ...
  function duplicateFile() { var current = activeFile(); if (!current) return; var copy = file(current.name.replace(/(\.[^.]+)?$/, ' copy$1'), current.text, current.folder); workspace.files.push(copy); openFile(copy.id); save(); }
  ...
  document.addEventListener('pointerdown', function (event) { if (!event.target.closest('.key[data-family]') && !event.target.closest('.order-tray')) $('orderTray').hidden = true; });
  document.addEventListener('keydown', function (event) { if (event.key === 'Escape') $('orderTray').hidden = true; });
  ...
  $('saveBtn').onclick = save; $('newFileBtn').onclick = createFile; $('newDocBtn').onclick = function () { createTemplate(templates[0]); }; $('newProjectBtn').onclick = showTemplates; $('templatesBtn').onclick = showTemplates; $('templatesActivity').onclick = showTemplates; $('filesActivity').onclick = toggleExplorer; $('searchActivity').onclick = function () { editor.focus(); }; $('settingsBtn').onclick = function () { $('inspector').classList.toggle('open'); }; $('closeTemplates').onclick = function () { $('templateDialog').close(); }; $('keyboardBtn').onclick = function () { $('keyboardPanel').classList.toggle('open'); }; $('closeKeyboard').onclick = function () { $('keyboardPanel').classList.remove('open'); }; $('toggleInspector').onclick = function () { $('inspector').classList.toggle('open'); }; $('closeInspector').onclick = function () { $('inspector').classList.remove('open'); }; $('addOutlineBtn').onclick = createFile; $('renameFileBtn').onclick = renameFile; $('duplicateFileBtn').onclick = duplicateFile; $('deleteFileBtn').onclick = deleteFile; $('undoBtn').onclick = function () { editor.focus(); document.execCommand('undo'); }; $('redoBtn').onclick = function () { editor.focus(); document.execCommand('redo'); }; $('findBtn').onclick = function () { $('findBar').classList.add('open'); $('findInput').focus(); }; $('closeFind').onclick = function () { $('findBar').classList.remove('open'); }; $('findNext').onclick = function () { findInDocument(1); }; $('findPrev').onclick = function () { findInDocument(-1); }; $('findInput').oninput = function () { findInDocument(1); };
  $('fontFamily').value = workspace.font || 'Noto Sans Ethiopic'; $('fontSize').value = String(workspace.size || 18); $('fontFamily').onchange = function () { workspace.font = $('fontFamily').value; editor.style.fontFamily = workspace.font; save(); }; $('fontSize').onchange = function () { workspace.size = Number($('fontSize').value); editor.style.fontSize = workspace.size + 'px'; save(); }; $('zoom').onchange = function () { document.querySelector('.document-paper').style.zoom = $('zoom').value; };
  document.querySelectorAll('[data-align]').forEach(function (button) { button.onclick = function () { editor.style.textAlign = button.dataset.align; }; });
