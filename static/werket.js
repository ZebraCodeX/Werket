(function () {
  ...
  function renderKeyboard() {
    ...
    host.innerHTML = html;
    host.querySelectorAll('[data-family]').forEach(function (button) { button.onclick = function (event) { event.stopPropagation(); host.querySelectorAll('.orders').forEach(function (old) { old.remove(); }); var family = button.dataset.family, pop = document.createElement('div'); pop.className = 'orders'; orders.forEach(function (order, index) { var choice = document.createElement('button'); choice.textContent = String.fromCodePoint(family.codePointAt(0) + index); choice.title = order === 'silent' ? 'Sixth order' : order; choice.onclick = function (click) { click.stopPropagation(); insert(choice.textContent); pop.remove(); }; pop.appendChild(choice); }); button.appendChild(pop); }; });
  ...
  document.addEventListener('pointerdown', function (event) { if (!event.target.closest('.key[data-family]') && !event.target.closest('.orders')) document.querySelectorAll('.orders').forEach(function (popup) { popup.remove(); }); });
  document.addEventListener('keydown', function (event) { if (event.key === 'Escape') document.querySelectorAll('.orders').forEach(function (popup) { popup.remove(); }); });
