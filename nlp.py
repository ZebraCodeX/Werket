...
def _next_words(last, second=''):
    ...
    if second and (key := second + '|' + last) in MODEL.get('trigram', {}):
        for word, count in MODEL['trigram'][key].items(): counts[word] = counts.get(word, 0) + count * 3
...
def check(text):
    ...
    for match in TOKEN.finditer(text or ''):
        word = match.group(0); known = fold(word) in KNOWN
        results.append({'word': word, 'start': match.start(), 'end': match.end(),
                        'known': known, 'suggestions': [] if known else _corrections(word)})
    return results

def suggest(text):
    text = text or ''; tokens = TOKEN.findall(text); partial = _partial(text)
    words = []
    if partial:
        prefix = fold(partial)
