...
class WerketHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/api/suggest':
            ...
            self._send(json.dumps(suggest(text), ensure_ascii=False), 'application/json; charset=utf-8')
            return
        if path == '/api/check':
            text = parse_qs(parsed.query).get('text', [''])[0]
            self._send(json.dumps({'text': text, 'words': check(text)}, ensure_ascii=False), 'application/json; charset=utf-8')
            return
        aliases = {'/manifest.json': '/static/manifest.json', '/sw.js': '/static/sw.js',
                   '/robots.txt': '/static/robots.txt', '/sitemap.xml': '/static/sitemap.xml',
