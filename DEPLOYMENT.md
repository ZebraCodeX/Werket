...
3. Render reads `render.yaml`, builds the container, and routes to its HTTP port
   (Render sets `PORT`; the container defaults to `10000`).
...
store assets already: 192/512 PNG icons, maskable icons, and an Apple touch
icon. Screenshots for store listings are generated by PWABuilder/the stores.
